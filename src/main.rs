...
impl iron::middleware::AfterMiddleware for RMS404Handler {
	...
	match err.response.status {
		...
		Ok(Response::with((status::NotFound, Path::new("./static/404.jpg"))))
	    },
	    _ => Err(err),
	}
    }
...
let resp = format!(r#"<!DOCTYPE html>
<!-- SPDX-License-Identifier: AGPL-3.0-only -->
<html>
	<head>
		<!--
...
title,
if js_enabled { "" } else { r#"<meta http-equiv="refresh" content="3;/">"# },
random_image());
    Ok(Response::with(
	(headers::ContentType::html().0, status::Ok, resp)
    ))
}

fn list_images() -> IronResult<Response> {
    Ok(Response::with(
	    (headers::ContentType::json().0, status::Ok, &*IMAGE_JSON.clone())
    ))
}

fn random_image() -> &'static str {
    rand::thread_rng().choose(&IMAGES)
	.unwrap()
}
...
fn images() -> Vec<String> {
	...
	match entry.file_type().expect("file type") {
		...
		// with '/' here.
		Some("/".to_string() + entry.path().strip_prefix("./static/").unwrap().to_str().unwrap())
	    },
	    _=> {
		None
	    }
